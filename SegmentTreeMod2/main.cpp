...
class SegmentTree{
    ...
    public:
        ...
        Node get(int id, int low, int high, int left, int right){
            if(low > right || high < left) return Node(0,0); // for sum or max of positive int
        ...
        void print(int id, int low, int high){
            if(low == high){
                printf("%d %d     ", ST[id].surplus0, ST[id].surplus1);
                return;
            }
...
int main(){
    ...
    while(q--){
        ...
        if(t==0){
            ...
            cout << st.get(1,1,n,l,r).surplus1 << endl;
            //st.print(1,1,n); cout << endl;
        }
    }
