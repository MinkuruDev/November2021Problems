...
class SegmentTree{
    private:
        ...
        vti mergeVector(vti vti1, vti vti2){
            if(vti1.size() == 0) return vti2;
            if(vti2.size() == 0) return vti1;
    ...
    public:
        ...
        vti get(int id, int low, int high, int left, int right){
            if(low > right || high < left) return vti(0); // for sum or max of positive int
            ...
            return mergeVector(get(id2, low, mid, left, right), get(id2+1, mid+1, high, left, right));
...
int main(){
    ...
    while(q--){
        cin >> l >> r;
        cout << st.get(1,1,n,l,r).size() << endl;
    }
